...
function getStrainMatrix(self::Truss, p::Array{Float64})
    local L = getLength(self);
    local Der = getShapeDerMatrix(self, p);
    local ndof = getDofNum(self);
    local nnode = getNodeNum(self);
    local B = zeros(Float64, 1, ndof);
    for i = 1 : nnode
...
function getStiffMatrix(self::Truss)
    local nnode = getNodeNum(self);
    local ndof = getDofNum(self);
    local Ke = zeros(Float64, ndof, ndof);
    local D = getStressStrainMatrix(self);
    local A = self.property.A;
    local n1 = self.nodes[1];
    local n2 = last(self.nodes);
    local n = Vector{Float64}([n2.x - n1.x, n2.y - n1.y, n2.z - n1.z]);
    local L = norm(n);
    n = n / L;
    ...
    local T = Matrix{Float64}(undef, ndof, ndof);
    for i = 1 : nnode
        T[i * 3 - 2, i * 3 - 2 : i * 3] = n';
    end
    return T' * Ke * T;
...
function getShapeDerMatrix(self::Truss2, p::Array{Float64})
    local nnode = getNodeNum(self);
    local Der = zeros(Float64, 1, nnode);
    Der[1, 1] = -0.5;
    Der[1, 2] = 0.5;
    return Der;
end
...
function getCoord(self::Element3D)
    local nnode = getNodeNum(self);
    local coord = zeros(Float64, nnode, 3);
    local i::Int32 = 1;
    for node in self.nodes
...
function getJacobi(self::Element3D, p::Array{Float64})
    local der = getShapeDerMatrix(self, p);
    local coord = getCoord(self);
    return der * coord;
end

function getStrainMatrix(self::Element3D, p::Array{Float64})
    local der = getShapeDerMatrix(self, p);
    local J = getJacobi(self, p);
    local Der = J^-1 * der;
    local ndof = getDofNum(self);
    local nnode = getNodeNum(self);
    local B = zeros(Float64, 6, ndof);
    for i = 1 : nnode
        local k = (i - 1) * 3 + 1;
        for j = 1 : 3
            B[j, (i - 1) * 3 + j] = Der[j, i];
...
function getStiffMatrix(self::Element3D)
    ...
    local D = getStressStrainMatrix(self);
    for p in self.points
        local B = getStrainMatrix(self, p);
        local J = getJacobi(self, p);
...
function getStressStrainMatrix(self::Element3D)
    local D = zeros(Float64, 6, 6);
    local E = self.property.M.E;
    local nu = self.property.M.nu;
    D[1, 1] = 1 - nu;
    D[1, 2] = nu;
    D[1, 3] = nu;
...
function getTriangleArea(nodes::Array{Node})
    local coord = Matrix{Float64}(undef, 3, 3);
    local k::Int32 = 1;
    for node in nodes
...
function getShapeDerMatrix(self::Tet4Element, p::Array{Float64})
    local nnode = getNodeNum(self);
    local der = zeros(Float64, 3, nnode);
    der[1, 1] = 1;
    der[2, 2] = 1;
    der[3, 3] = 1;
    der[1, 4] = -1;
...
function Tet10Element(nodes::Array{Node}, property::Property3D)
    local alpha = 0.58541020;
    local beta = 0.13819660;
